# We're starting with the same base image, but we're declaring that this block outputs an image called DEPS that we won't be deploying - it just installs our Yarn deps
FROM node:14-alpine AS deps

# If you need libc for any of your deps, uncomment this line:
RUN apk add --no-cache libc6-compat

# Copy over ONLY the package.json and yarn.lock so that this `yarn install` layer is only recomputed
...
WORKDIR /app
# Bring over the deps we installed and now also the rest of the source code to build the Next server for production
